...
description: "Use for building or maintaining a Zotero research library from a topic, DOI list, title list, or Excel catalog: discover with OpenAlex, validate with Crossref, deduplicate in Zotero, preview before writes, acquire only lawful full text, queue browser authentication, and produce a complete report through the paired Literature MCP."
---
...
5. 输出候选预览，至少分为“将新增、Zotero 已存在、匹配不确定、查询失败”，列出推荐理由和建议 Collection、Tags、Priority。
6. 等待用户明确确认待写入集合。任何多条写入都视为批量操作，必须先预览；用户对旧预览的确认不能自动扩展到新候选。
7. 写入前再次快速去重，然后通过 Literature MCP 调用 `create_collection`、`add_paper_by_doi`、`add_tags`、`add_note`。仅处理用户已确认的条目。
...
- 不自动删除 Zotero 文献、Collection、附件或笔记。发现错误条目或问题附件时，只列出精确对象、证据、影响和建议删除理由，并询问用户；没有明确授权不得删除。
...
- 候选预览和最终报告都显示 DOI、标题、年份、来源、去重状态、匹配置信度、推荐理由、Collection、Tags、Priority、全文状态和下一步。
